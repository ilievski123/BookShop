...
class Product
{
    ...
    public function create()
    {
        ...
        if (empty($data['nameProduct'])) {
            $this->error .= "Внеси име! <br>";
        }

        if (empty($data['descriptionProduct'])) {
            $this->error .= "Внеси опис! <br>";
        }

        if (empty($data['priceProduct'])) {
            $this->error .= "Внеси цена! <br>";
        }

        if (empty($data['stockProduct'])) {
            $this->error .= "Внеси количина! <br>";
        }

        if (empty($data['idCategoryProduct'])) {
            $this->error .= "Внеси категорија на книгата! <br>";
        }

        if (empty($data['imageProduct'])) {
            $this->error .= "Внеси слика! <br>";
        }
        ...
        if (is_array($products)) {
            foreach ($products as $product) {
                $tableHTML .= '<tr>
                            ...
                            <td><button class="btn btn-primary"><a href=products/update/' . $product->idProduct . '>Промени</a></button></td>
                            <td><button class="btn btn-warning"><a href=deleteProduct/' . $product->idProduct . '>Избриши</a></button></td>
                        </tr>';
            }
        }
        return $tableHTML;
                ...
                $html .= '<div class="col-12 col-sm-6 col-lg-4 my-3">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <a href="' . ROOT . 'products/details/' . $product->idProduct . '" class="btn btn-info">Види</a>
                                </div>
                            </div>
                        </div>';
            }
